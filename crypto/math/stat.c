...
stat_test_poker(uint8_t *data) {
  ...
  debug_print(mod_stat, "poker test: this test not supported because double is not supported in linux kernel", NULL);
...
stat_test_rand_source(rand_source_func_t get_rand_bytes) {
 int i;
  ...
  #else
  debug_print(mod_stat, "stat: skipped poker test because double is not supported in linux kernel", NULL);
  #endif
